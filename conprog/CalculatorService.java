...
public class CalculatorService extends Thread{
    private Socket serverSocket = null; private String requestStr = null; private String outcome = null;
    //Class constructor
    public CalculatorService(Socket aSocket) { //Launch the calculator service thread //TO BE COMPLETED
    }
    // Retrieve the request from the socket
    public String retrieveRequest() { //TO BE COMPLETED
        return this.requestStr;
    }
    //Parse the request command and execute the calculation
    public boolean attendRequest() { //TO BE COMPLETED
    ...
    public void returnServiceOutcome() { //TO BE COMPLETED
    }
    //The service thread run() method
    public void run() { try {
// Retrieve the service request from the socket
        this.retrieveRequest(); System.out.println("Service thread " +
                        this.getId() + ": Request retrieved: " + this.requestStr);
// Attend the request
        boolean tmp = this.attendRequest(); //Send back the outcome of the request if (!tmp)
        System.out.println("Service thread " + this.getId() + ": Unable to provide service.");
        this.returnServiceOutcome(); }catch (Exception e){
